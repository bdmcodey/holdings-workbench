...
- **Holdings Data--General Information** (`hd853878.html`) — all three files
  cross-reference it for compressibility and for how the four field types
  relate. The missing spine.
- **Leader** (`hdleader.html`) — Leader/17 decides the 863 first indicator,
  which is why that indicator is still an open question here rather than a
  fixed value. Noted since D18.
- **Appendix B: Full Level Record Examples** (`examples.html`) — whole records
  to check output shape against, rather than fields in isolation.
